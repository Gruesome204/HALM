...
public class GridManager : MonoBehaviour
{
    ...
    void Awake()
    {
        if (Instance == null)
        {
            ...
            // Calculate the offset to center the grid at the origin (0, 0)
    ...
    {
        Vector2 localPosition = worldPosition - gridOffset;
        int gridX = Mathf.FloorToInt(localPosition.x / cellSize);
        int gridY = Mathf.FloorToInt(localPosition.y / cellSize);
        ...
        if (Instance == this) // Only draw if this is the active instance
        {
            for (int x = 0; x < width; x++)
            {
                for (int y = 0; y < height; y++)
                {
                    ...
                    Gizmos.DrawWireCube(cellCenter, new Vector3(cellSize, cellSize, 0.1f)); // Keep as a thin cube for visibility in 2D scene
                }
            }
        }
    }
